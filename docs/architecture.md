...
ActionRegistry
    ...
    ├── agents.*     one-shot + persistent Agent lifecycles
...
- Child Pi processes load normal extensions by default so installed model-provider plugins (including Makora-like providers) continue to work without a Fabric allowlist. Claude children use the official installed CLI and its existing authentication. Both runners restrict the active model-facing tools to `defaultTools`; Pi adds `fabric_exec` only for explicit recursion, while Claude rejects recursion and unmapped tools.
...
- `/fabric prewalk` selects a Pi executor through `prewalk.model` or the interactive picker. Research mode is the research-compatible path: a hidden phase-owned instruction requires a host-accepted 5–9 item `prewalk.checklist`, matching mutations are rejected until readiness, and QuickJS or Node execution is stopped after the first successful matching audit so later guest code does not run. The planning instruction is filtered from model context before the identity-owned continuation switches the same Main session to the executor, which retains model ownership through implementation and verification. In-place and trajectory modes run the executor as an off-session child so Main keeps its own model; both preserve the completed-outer-`fabric_exec` boundary. Bash remains opaque unless its exact ref is configured.
...
- Claude model discovery uses a local initialization control request and does not invoke a model. Actual one-shot and persistent Agent activations use the account/API billing already configured in Claude Code; Fabric records the CLI's reported `total_cost_usd` in normal usage and budget ledgers.
...
- Background one-shot children are stopped when the parent Pi session shuts down. A detached `agents.spawn()` sends a follow-up completion message unless the caller later waits for it or `notifyOnComplete` is disabled. Durable participant lifecycle subscriptions provide source-qualified Pi/run notifications across roots and Agents without transcript disclosure; their mesh cursors prevent historical replay and survive owner restarts. Completed worktrees are intentionally retained.
- Persistent agents are suspended on shutdown and restored when project trust is active. Claude persistent Agent session IDs refer to Claude Code's own persisted session store; removing that private session makes resume fail, and removing a Fabric Agent does not currently delete Claude Code's private transcript. By default (`mesh.persistentAgentScope: "project"`), definitions, mailbox history, and child session files live under `.pi/fabric/mesh/persistentAgents/` and are shared across Pi sessions, so persistent Agents survive `/new`. Set `mesh.persistentAgentScope: "session"` to isolate them per Pi session. Mesh topics and shared state are always project-scoped. Do not place secrets in Agent prompts, messages, or mesh state.
- Approving `agents.create()` delegates future subscribed events to that persistent Agent until it is stopped. Each activation uses the Agent's fixed runner and persisted tool allowlist/model setting; review them before approving a persistent agent. Tool changes apply only to later activations.
- Persistent Agent responses can enter the main context only through the delivery policy fixed at creation. Directive output is schema-validated, but it is still untrusted model output that the main agent should weigh.
- Project-scoped persistent Agent execution is ownership-gated: passive sessions do not consume events, run mailboxes, expose private Agent data, mutate settings, or persist shutdown state, and they reload the registry after acquiring ownership. Shared `persistentAgents.json` updates use a stale-safe lock plus ownership-aware read/merge/write, so one owner cannot overwrite another owner's newer records. Use session scope when roots should define fully independent persistent Agent sets. Mesh events are append-only until bounded log compaction retains the newest tail; archive or remove an old mesh root when even that retained history is no longer useful.
