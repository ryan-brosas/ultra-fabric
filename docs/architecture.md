...
- `/fabric prewalk` selects a Pi executor through `prewalk.model` or the interactive picker. A hidden phase-owned instruction requires a host-accepted 5-9 item `prewalk.checklist`, matching mutations are rejected until readiness, and QuickJS or Node execution is stopped after the first successful matching audit so later guest code does not run. The planning instruction is filtered from model context before the identity-owned continuation switches the same Main session to the executor. Main is restored to its original model when the continuation settles. Bash remains opaque unless its exact ref is configured.
