...
`prewalk.fallbackModels` is an optional ordered list of at most eight unique Pi `provider/model` keys. The host tries each model in order before switching Main, so a configured fallback covers model outages without losing the session. The selected fallback is visible in the result and lifecycle. A failed switch keeps the accepted plan armed with its task, checklist, and attempt, so the next matching mutation retries the handoff; Main never left the frontier model, so nothing is restored and nothing needs an operator action.
