...
| Model ownership | Prewalk switches the same Main session at the first successful mutation, keeps the executor through implementation and verification, and hands Main back its own model on settle. | Automatic nested planner/executor stacks remain out of scope. |
