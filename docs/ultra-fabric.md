...
Slices 0-7 now have tested backend contracts. Prewalk v2 owns typed continuity, configurable effect triggers, identity-checked continuation, bounded execute → verify → revise, with a host-accepted 5–9 item checklist, checklist-triggered runtime stop, planning-message pruning, and same-session executor ownership that hands Main back its own model on settle. Persistent Agents own durable inbox/outbox replay, retry/dead-letter/circuit behavior, quotas, telemetry, root traces, and outcomes. Fabric propagates run identity, gates, reservations, durable phase leases, cooperative path leases, deterministic Context QoS, capability/auth routing, explicit delegation intent, capability profiles, and a prompt-free outcome ledger with confidence-bounded recommendations. Automatic policy promotion remains disabled until the Slice 8 benchmark and soak gates are run on representative real tasks. The first real collector runs (2026-08-08) proved the omniroute provider route headless and fixed harness gaps (planningEscapes, checklist trace projection, boundary detection, extra-extension loading), but the RPC executor continuation does not run and the bundled corpus is too small for the research protocol, so the benchmark and soak gates remain unmet and promotion stays off. Prewalk protocol compatibility does not inherit external cost, quality, or success-rate claims. A trivial-path escape, adopted from the opencode-prewalk frontier triviality check and the hermes single-edit philosophy, lets a task that clearly fits in one or two small edits record `prewalk.checklist({ trivial: true })` and complete directly: the controller skips the mutation boundary and the executor handoff, so no model swap fires for trivial work. Deferred prewalk backlog: stateless checkpoint recovery after a host restart (requires reconstructing the arm from the session, per opencode-prewalk F2), named preset slots with per-phase planner/executor effort and a bounded planner budget (hermes presets.yaml), and a single-todo no-handoff guardrail (opencode-prewalk P5). The offline `benchmark:prewalk` analyzer now enforces strict prompt-free records, paired in-place/research coverage across at least 20 operator-attested representative tasks, provenance, protocol evidence, and confidence reporting. The opt-in `benchmark:prewalk:real` collector adds isolated paired Pi RPC runs, exact filesystem/test oracles, prompt-free probe telemetry, a non-billable evaluator seam, and an observed USD stop. A bundled 20-task source-mapped contract corpus is offline RED/GREEN qualified and defaults to non-representative; independently sampled historical work and empirical results remain Slice 8 gates. Prewalk planning now projects a typed Schema-first contract (intent, reference questions with evidence refs, local scope, invariants, postconditions): Schema is the progression authority, CGC serves explicit reference research, local Codemap supplies scope and structural validation, and global `schema.mode` stays off until the Slice 8 benchmark and soak gates promote enforcement. The quality increment adds opt-in host-owned changed-file gates with broad language detection, trusted shell-free checks, explicit coverage failure, and distinct failed, timed-out, and crashed outcomes.
...
The agent posture is explicit, on-request utilization: recon and research stay on Main's context, `prewalk.autoScout` is explicit opt-in and runs a cheap scout child before planning only when enabled, and support roles or `consult.run` workers run only through an explicit call. Every child stays bounded by admission (consult admits up to `consult.maxWorkers`, 3 by default), turn budgets, and spend caps. Power is available, not ambient.
...
| Context QoS | Every model request gets a deterministic pre-threshold pass that retires large, old, superseded read/grep/find/ls bodies and, above a configured oversized ceiling, any old successful retirable-tool result (including fabric_exec and codemap) even without a newer equivalent, while preserving message count, call/result pairing, recent turns, errors, mutations, non-text content, and typed Fabric evidence. Cumulative omission counters are host-visible. A message-count ceiling compacts proactively before dispatch so a long message-dense session is not rejected by a provider history cap. | Semantic compression and model-authored summaries remain outside this pass. |
...
This increment closes the context-capacity and fresh-subagent gap identified after Slices 0-7 by making bounded delegation available on request (autoScout explicit opt-in, consult.run zero-worker admission by default, ledger-attributed). It does not prove that delegation improves outcomes. Automatic policy expansion remains blocked on the Slice 8 benchmark.
...
- Delegated and exploration results stay bounded at the source; oversized old retirable-tool results are retired deterministically before the next model request rather than inflating context until compaction.
- A message-count ceiling compacts before dispatch, so provider history caps (for example 800 messages) reject no request and a rejected session recovers with one manual /compact.
