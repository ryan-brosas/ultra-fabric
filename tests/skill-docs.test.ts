...
describe("fabric-exec skill provider contracts", () => {
  ...
  it("retains each specialized skill's execution invariants", () => {
    const required: Record<string, string[]> = {
      ...
      "fabric-exec": ["read, describe, retry", "tools.describe", "timeoutMs", "multiline or syntax-heavy payloads", "shell heredocs", "never load them autonomously", "Peer is a reserved Fabric term", "query `agents.peers()` first", "Search before reading", "offset", "2000 lines or 50KB", "Use offset=n to continue", "consult.run", "zero workers", "Partition", "Challenge", "Compare", "at most one", "host-resolvable"],
