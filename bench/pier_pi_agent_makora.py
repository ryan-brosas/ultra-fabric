...
class PiCodingAgent(BaseInstalledAgent):
    ...
    async def run(
        ...
        if self._fabric_package_name:
            ...
            provider_flag = "--no-skills --no-extensions"
            if self.model_name.startswith("makora/"):
                provider_dir = (
                    Path("/tmp/pi-agent") / "npm" / "node_modules" / "pi-makora-provider"
                )
                provider_flag += f" -e {shlex.quote(str(provider_dir))}"
            extension_flags = provider_flag
        command = " ".join(
            [
