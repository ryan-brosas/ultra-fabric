...
PI_BENCH_PROVIDER=${PI_BENCH_PROVIDER:-makora}
PI_BENCH_MODEL=${PI_BENCH_MODEL:-makora/deepseek-ai/DeepSeek-V4-Flash}
...
provider = os.environ.get("PI_BENCH_PROVIDER", "makora")
model = os.environ.get("PI_BENCH_MODEL", "makora/deepseek-ai/DeepSeek-V4-Flash")
selected = {key: auth[key] for key in (provider,) if key in auth}
if not selected:
    raise SystemExit(f"{provider} credentials are unavailable in {auth_path}")
...
with open(os.path.join(agent_dir, "settings.json"), "w") as handle:
    json.dump({
        "defaultModel": model.split("/", 1)[1],
        "defaultProvider": provider,
        "defaultThinkingLevel": "low",
        "packages": ["npm:pi-makora-provider"] if provider == "makora" else [],
    }, handle)

# Vendored provider: pi loads npm packages from <agent dir>/npm/node_modules.
# Fabric is installed globally in the container by the agent install spec.
if provider == "makora":
    srcp = os.path.expanduser("~/.pi/agent/npm/node_modules/pi-makora-provider")
    dstp = os.path.join(agent_dir, "npm", "node_modules", "pi-makora-provider")
    if os.path.isdir(srcp):
        ...
        print("warning: pi-makora-provider not found at " + srcp)
PY
chmod 600 "$AGENT_DIR"/auth.json "$AGENT_DIR"/settings.json
...
PIER_ARGS=(
  ...
  --path "$TASK_PATH"
  --agent-import-path pier_pi_agent_makora:PiCodingAgent
  --model "$PI_BENCH_MODEL"
