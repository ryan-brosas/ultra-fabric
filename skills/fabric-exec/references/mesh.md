...
`mesh` is a project-scoped, event-sourced coordination substrate. With persistent agents (see `agents.md`) it is sufficient to express messenger-style swarms without a daemon or fixed planner/worker roles. For the sandbox model, see the parent `fabric-exec` skill.
...
- `mesh.self()` returns the caller's wire identity `{ id, name, kind, sessionId? }`; use `agents.self()` when lifecycle-aware participant metadata is needed.
- `mesh.members({ scope?, kinds?, includeStale?, limit? })` returns the same `FabricParticipantInfo[]` as `agents.members`: roots and Agents (`kind: "agent"` plus lifecycle metadata) with `rootId`, optional `parentId`, `ownerHostId`, status, capabilities, and `local`/`stale` flags. `scope` defaults to `"project"`.
- `agents.self()` is the richer participant identity; `agents.list({ lifecycle })` is the Agent inventory. `agents.main()` and `agents.peers()` are compatibility views derived from root participants.
...
Each entry is `{ key, value, version, updatedAt, updatedBy }`. Use `ifVersion` for task claims, leases, reservations, and decisions: create with `ifVersion: 0`, claim by passing the current `version`. Keys below `topology/`, `sessions/`, and `persistentAgents/` are host-reserved and reject guest `put`/`delete`; use an application prefix such as `tasks/` or `team/`.
...
- Persistent agents subscribe to topics via `agents.create({ topics: [...] })` (see `agents.md`); published events are delivered as `mesh:<topic>` messages.
- Set `mesh.enabled: false` in config to disable both mesh actions and ambient persistent agent restoration.
