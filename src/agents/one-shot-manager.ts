...
export class OneShotAgentManager {
  ...
  async spawn(request: AgentRunRequest, signal?: AbortSignal): Promise<AgentHandleInfo> {
    ...
    try {
      ...
      const role = normalizeFabricAgentRole(request.role);
      ...
      const workerArguments = [
        ...
        ...(request.persistentAgentId ? ["--persistentAgent-id", request.persistentAgentId] : []),
        ...(request.persistentAgentName ? ["--persistentAgent-name", request.persistentAgentName] : []),
        ...(request.runContext
          ? [
              "--trace-id",
      ...
      const managed: ManagedAgent = {
        ...
        ...(thinking ? { thinking } : {}),
        ...(request.persistentAgentId ? { persistentAgentId: request.persistentAgentId } : {}),
        ...(request.persistentAgentName ? { persistentAgentName: request.persistentAgentName } : {}),
        ...(request.runContext
          ? {
  ...
  async #runRetentionSweep(now = Date.now()): Promise<void> {
    ...
    const expired = [...this.#runs.values()].filter((managed) => {
      if (!managed.settled || managed.persistentAgentId) return false;
      const record = readRecord(managed.statusFile) ?? managed.latestRecord;
  ...
  #emitLifecycle(
    ...
    try {
      this.#onLifecycle({
        source: {
          id: managed.persistentAgentId ?? managed.id,
